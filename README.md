...
The site includes a **High contrast** control in the header. Its setting is stored in the browser and restored on subsequent page loads, including direct navigation to another page. The control is keyboard accessible and exposes its current state through `aria-pressed`.
